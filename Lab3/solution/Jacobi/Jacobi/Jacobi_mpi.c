...
int main ( int argc, char ** argv ) {
    ...
    //Initialization of omega which will not be needed in Jacobi
    ...
    //---Define the iteration ranges per process-----//
        ...
        */
        ...
        /*
        if clauses - boundary process with or without padding for this side

        i_min and j_min equal to 2 because we have boundary process and no added row or column from neighbour process.
        Padding can appear only in boundary processes and if there is padding it will only affect the south and east sides (i_max or/and j_max).
        If there is no padding in a boundary process then (global_padded[0] - global[0]) = 0 so (i_max or/and j_max) will be correct in this case as well.

        else clauses - internal process for this side

        i_min and j_min equal to 1 (not 2 - now we have one more row/column) because we have one added row or column from neighbour process.
        i_max and j_max equal to (local[0] + 1) because we have one added row or column from neighbour process.
        */
        ...
        for (t=0;t<T;t++) {
                ...
                //communicate with neighbours - send and receive all needed rows and columns

                counter = 0;
                if (north != MPI_PROC_NULL) {
                        MPI_Isend(&u_previous[1][1], 1, row, north, rank, CART_COMM, &requests[counter++]);
                        MPI_Irecv(&u_previous[0][1], 1, row, north, north, CART_COMM, &requests[counter++]);
                }

                if (south != MPI_PROC_NULL) {
                        MPI_Isend(&u_previous[local[0]][1], 1, row, south, rank, CART_COMM, &requests[counter++]);
                        MPI_Irecv(&u_previous[local[0]+1][1], 1, row, south, south, CART_COMM, &requests[counter++]);
                }

                if (west != MPI_PROC_NULL) {
                        MPI_Isend(&u_previous[1][1], 1, column, west, rank, CART_COMM, &requests[counter++]);
                        MPI_Irecv(&u_previous[1][0], 1, column, west, west, CART_COMM, &requests[counter++]);
                }

                if (east != MPI_PROC_NULL) {
                        MPI_Isend(&u_previous[1][local[1]], 1, column, east, rank, CART_COMM, &requests[counter++]);
                        MPI_Irecv(&u_previous[1][local[1]+1], 1, column, east, east, CART_COMM, &requests[counter++]);
                }
    ...
    if (rank==0) {
        printf("Jacobi X %d Y %d Px %d Py %d Iter %d ComputationTime %lf TotalTime %lf ConvergedTime %lf midpoint %lf\n",global[0],global[1],grid[0],grid[1],t,comp_time,total_time,conv_time,U[global[0]/2][global[1]/2]);

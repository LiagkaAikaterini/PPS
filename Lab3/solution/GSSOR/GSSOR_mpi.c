...
int main(int argc, char ** argv) {
    ...
    struct timeval tts,ttf,tcs,tcf,tcvf,tcvs;   //Timers: total-> tts,ttf, computation -> tcs,tcf
    ...
    for (t=0;t<T;t++) {
        ...
        if (north != MPI_PROC_NULL) {
                MPI_Isend(&u_previous[1][1], 1, row, north, rank, MPI_COMM_WORLD, &requestsfirst[first++]);
                MPI_Irecv(&u_current[0][1], 1, row, north, north, MPI_COMM_WORLD, &requestsfirst[first++]);
        }

        if (south != MPI_PROC_NULL) {
                MPI_Irecv(&u_previous[local[0]+1][1], 1, row, south, south, MPI_COMM_WORLD, &requestsfirst[first++]);
        }

        if (west != MPI_PROC_NULL) {
                MPI_Isend(&u_previous[1][1], 1, column, west, rank, MPI_COMM_WORLD, &requestsfirst[first++]);
                MPI_Irecv(&u_current[1][0], 1, column, west, west, MPI_COMM_WORLD, &requestsfirst[first++]);
        }

        if (east != MPI_PROC_NULL) {
                MPI_Irecv(&u_previous[1][local[1]+1], 1, column, east, east, MPI_COMM_WORLD, &requestsfirst[first++]);
        }
        ...
        //counter = 0;
        if (south != MPI_PROC_NULL) {
            MPI_Isend(&u_previous[local[0]][1], 1, row, south, rank, MPI_COMM_WORLD, &requestslast[last++]);
        }

        if (east != MPI_PROC_NULL) {
            MPI_Isend(&u_previous[1][local[1]], 1, column, east, rank, MPI_COMM_WORLD, &requestslast[last++]);
        }
	...
	printf("GaussianSeidelSOR X %d Y %d Px %d Py %d Iter %d ComputationTime %lf TotalTime %lf ConvergedTime %lf midpoint %lf\n",global[0],global[1],grid[0],grid[1],t,comp_time,total_time,conv_time,U[global[0]/2][global[1]/2]);
